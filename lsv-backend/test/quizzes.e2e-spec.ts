...
describe('Quizzes (e2e)', () => {
    ...
    beforeAll(async () => {
        ...
        // Prep data
        const langRes = await request(app.getHttpServer()).post('/languages').set('Authorization', `Bearer ${adminToken}`).send({ name: 'Quiz Lang', description: 'Test', countryCode: 'US' });
        const stageRes = await request(app.getHttpServer()).post('/stage').set('Authorization', `Bearer ${adminToken}`).send({ name: 'Quiz Stage', description: 'Test', languageId: langRes.body.id });
    ...
    describe('Quiz CRUD', () => {
        ...
        it('/quiz/:quizId/submission (POST) - Should calculate correct score', async () => {
            ...
            const correctOptionId = quiz.body.questions[0].options.find(o => o.isCorrect).id;
            const incorrectOptionId = quiz.body.questions[0].options.find(o => !o.isCorrect).id;
