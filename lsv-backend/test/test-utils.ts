 ...
 */
export async function getAdminToken(app: any, dataSource: DataSource): Promise<string> {
    ...
    if (!admin) {
        // Importamos BcryptService dinámicamente o usamos un hash conocido
    ...
    const response = await require('supertest')(app.getHttpServer())
        .post('/auth/login')
        .send({ email: adminEmail, password: adminPassword });

    if (!response.body || !response.body.data || !response.body.data.token) {
        console.error('Login failed! Response:', response.status, JSON.stringify(response.body, null, 2));
        throw new Error(`Login failed for ${adminEmail} with status ${response.status}`);
    }
 ...
 */
export async function getUserToken(app: any, dataSource: DataSource): Promise<string> {
    const userEmail = `test-user-${Date.now()}@example.com`;
    const userPassword = 'password123';

    // Registro
    await require('supertest')(app.getHttpServer())
        .post('/auth/register')
        .send({
            email: userEmail,
...
export async function getModeratorToken(
    ...
    // 1. Registro
    const regRes = await require('supertest')(app.getHttpServer())
        .post('/auth/register')
        .send({
            email: modEmail,
