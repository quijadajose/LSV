...
describe('Moderators (e2e)', () => {
    ...
    beforeAll(async () => {
        ...
        const testUser = { email: 'mod-user@example.com', password: 'password123', firstName: 'Mod', lastName: 'User', age: 20, isRightHanded: true };
        ...
        const langRes = await request(app.getHttpServer()).post('/languages').set('Authorization', `Bearer ${adminToken}`).send({ name: 'Mod Lang', description: 'Test', countryCode: 'US' });
    ...
    describe('Moderator Permissions', () => {
        ...
        it('/admin/moderators (POST) - Should FAIL if not admin', async () => {
            const userToken = await require('./test-utils').getUserToken(app, dataSource);
