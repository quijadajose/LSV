...
describe('User Lessons (e2e)', () => {
    ...
    beforeAll(async () => {
        ...
        const langRes = await request(app.getHttpServer()).post('/languages').set('Authorization', `Bearer ${adminToken}`).send({ name: 'ULang', description: 'Test', countryCode: 'US' });
        const stageRes = await request(app.getHttpServer()).post('/stage').set('Authorization', `Bearer ${adminToken}`).send({ name: 'UStage', description: 'Test', languageId: langRes.body.id });
        ...
        const testUser = { email: 'user-ulesson@example.com', password: 'password123', firstName: 'U', lastName: 'L', age: 20, isRightHanded: true };
        const regRes = await request(app.getHttpServer()).post('/auth/register').send(testUser);
